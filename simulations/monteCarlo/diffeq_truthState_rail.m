...
R_b2f = q2dcm(q_b2f);
g_mag = calcGrav(h, simpar.init.lat);
...
F_b = [F_thrust - qbar*A_ref*C_D; %7.6.23 of Phillips does not include cos(beta)
    0;
    0];

% Calculate external forces
F_e = [-g_mag*(sin(simpar.init.elevation) + C_f*cos(simpar.init.elevation));
    0;
    0];
...
rdot_f = R_b2f*v_b;
vdot_b = F_b/m + F_e - cross(w_b, v_b) + n_nu;
